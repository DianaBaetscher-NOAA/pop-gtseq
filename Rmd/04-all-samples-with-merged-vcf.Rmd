...
# 
# rockfish_meta %>%
#   write_rds("../data/rockfish_metadata.rds")

pop <- read_rds("../data/rockfish_metadata.rds")
...
# add metadata for Tony's samples
tony_meta_missing <- POP_meta_ss %>%
  filter(DonatingLab == "Tony Gharrett") %>%
  select(CollectionYear, CollectionMonth, CollectionDay, StartLatitudeDD, StartLongitudeDD, CruiseID, HaulID, StationID, DepthM, LengthMM, DonatingLab) %>%
  unique() #%>%
  #mutate(CruiseID = str_replace(CruiseID, "-", ""))

#tony_meta_missing$CruiseID <- as.numeric(tony_meta_missing$CruiseID)
...
bt_meta <- read_csv("../data/FOSS_bottomTrawl_haulData.csv") %>%
  select(Year, Cruise, Haul, Station, `Latitude Dd`, `Longitude Dd`, `Depth M`) %>%
  mutate(StartLatitudeDD = round(`Latitude Dd`, 2)) %>%
  mutate(StartLongitudeDD = round(`Longitude Dd`, 2))

tony_fixed_meta <- tony_meta_missing %>%
  left_join(., bt_meta, by = c("CollectionYear" = "Year", "HaulID" = "Haul", "StartLatitudeDD", "StartLongitudeDD")) %>%
  mutate(StationID = ifelse(StationID == "---", Station, StationID)) %>%
  mutate(DepthM = ifelse(DepthM == "0" | is.na(DepthM), `Depth M`, DepthM))

tony_fixed_meta %>%
  inner_join(., POP_meta_ss) %>%
  filter(!is.na(DepthM))
...
POP_meta_ss %>%
  ...
  filter(SpeciesName == "Sebastes alutus") %>%
  group_by(CruiseID, HaulID, StationID, CollectionYear, DonatingLab, StartLatitudeDD, StartLongitudeDD) %>%
  tally() %>%
  arrange(CollectionYear) %>%
  filter(DonatingLab != "SWFSC")


```
additional metadata for SWFSC samples? Maybe not critical because they're basically all B4 type.

```{r}
bt_meta %>%
  filter(Year == 2000)

# begin adding in the fixed metadata
POP_meta_ss %>%
  mutate(DepthM = ifelse(CollectionYear == 1999 & CruiseID == 199901 & HaulID == 293, 274, DepthM))


```
Some of this data matches up and some doesn't with the bottom trawl. I don't love that.
...
```{r quick-look-at-weird-B2-sample}
grps_for_map %>%
  filter(grp == "B2") %>%
  filter(StartLatitudeDD < 45)
...
l <- baseline_meta %>%
  ...
  labs(x = "Sebastes alutus, genetic group",
       y = "Length (mm)",
       title = "Sebastes alutus, collection length by genetic group") +
  theme(
    axis.title.x = element_text(margin = margin(t = 10)),
    axis.title.y = element_text(margin = margin(r = 10))
  ) +
...
```{r depth-genetic-groups-space}
depth_cohort <- baseline_meta %>%
  anti_join(., YOY_baseline) %>%
  group_by(grp) %>%
  filter(!is.na(DepthM)) %>%
  filter(DepthM > 200 & DepthM < 250) %>%
  filter(grp %in% c("A", "C", "D")) %>% # just these groups
    filter(LengthMM > 220)
  
  
goa_map +
  geom_jitter(data = depth_cohort, aes(x = StartLongitudeDD,y = StartLatitudeDD, color = grp), size = 3, alpha = 0.7, height = 0.2, width = 0.2) 
  labs(x = "Longitude",
       y = "Latitude",
       title = "Sebastes alutus depth cohort test \n Depth = 200-250 m") +
  scale_color_manual(values = c("darkcyan", "deepskyblue", "salmon", "darkblue", "sienna", "slateblue", "darkorange","firebrick", "purple4")) +
  theme(
    axis.text = element_text(size = 8),
    axis.title.x = element_text(margin = margin(t = 10)),
...
ggsave("pdf_outputs/depth_cohort_test.pdf")
```
...
# quick look at distribution of mixture samples
#convert_to_genind_pca(mix_grps_for_pca, "mixture_plus_BASEv2")
...
mix_df_for_genind_w_assignments <- pca_df_for_conversion %>%
  ...
  mutate(STRATA = ifelse(STRATA == "POP", repunit, STRATA)) %>%
  filter(STRATA != "POP") %>%
  select(-repunit)
...
goa_map +
  ...
  labs(x = "Longitude",
       y = "Latitude",
       title = "Sebastes alutus mixture samples - rubias group assignments from GTseq panel") +
  scale_color_manual(values = c("darkcyan", "deepskyblue", "salmon", "darkblue", "sienna", "slateblue", "darkorange","firebrick", "purple4")) +
  theme(
    axis.text = element_text(size = 8),
    axis.title.x = element_text(margin = margin(t = 10)),
...
mix_assign_w_meta %>%
  filter(is.na(DepthM)) %>%
  ungroup() %>%
  select(StartLatitudeDD, StartLongitudeDD, HaulID, StationID, DonatingLab) %>%
  unique()
...
Ok, I'm trying to track down info for 53 unique lat/lon/haul/station/lab batches. That seems more manageable. Although it wasn't in the POP metadata from Tony that I had in emails. Perhaps that will take an ask to Tony with the specifics.
...
```
Very few samples with depth info there, but the pattern holds between A and D for those that are present.


I need to see if I can track down more depth data based on the year/cruise/haul/station ID info.


I couldn't find additional metadata, but at least I can still take a look at per-haul ratios by geography.

```{r load-packages-for-scatterpie}
library(scatterpie)
...
# map
# goa_map +
#   geom_scatterpie(aes(x = StartLongitudeDD,y = StartLatitudeDD, color = collection, r = multiplier*6), 
#                    data = hauls, cols = )) +
  #geom_jitter(data = hauls, aes(x = StartLongitudeDD,y = StartLatitudeDD, color = collection), size = 1, alpha = 0.5, height = 0.2, width = 0.2) +
  #facet_wrap(~collection) +
  labs(x = "Longitude",
       y = "Latitude",
       title = "Sebastes alutus mixture samples - rubias group assignments from GTseq panel") +
...
# geom_scatterpie(aes(x = StartLongitudeDD,y = StartLatitudeDD, color = collection, r = multiplier*6), 
#                   data = hauls, cols = colnames(final_data[,c(2:11)]))
