...
TO DO:
- Still need to clean-up metadata more.
- Need to do a matching genotype analysis to determine if samples with different ABLG numbers are actually the same sample.
...
loc136_list <- genos_locs_filtered %>%
  select(locus) %>%
  unique()
```
...
```{r quick-check-of-primer-pool}
pool <- read_tsv("../data/20230130POP_GTseq3_JanusFile.txt")

poola <- pool %>%
  mutate(Locus = str_replace(Locus, "_F", "")) %>%
  mutate(Locus = str_replace(Locus, "pop-gtseq_", ""))

loc136_list %>%
  anti_join(., poola, by = c("locus" = "Locus"))
```
...
### output baseline genotypes for GSI

```{r extract-baseline-genotypes-for-GSI}
clean_ascertain_ids$ABLG <- as.numeric(clean_ascertain_ids$ABLG)
baseline_genos <- genos_locs_ind_filtered %>%
  inner_join(., clean_ascertain_ids)

baseline_genos %>%
  write_rds("../data/processed/gsi_baseline_genotypes.rds")
```
...
ref_two_col$indiv <- as.character(ref_two_col$indiv)
```
...
mix_two_col$indiv <- as.character(mix_two_col$indiv)
...
ggsave("pdf_outputs/pop_grp_depth_dist_rubias_assignments.png", width = 7, height = 5)
...
### Mixtures by haul
```{r duplicated-metadata?}
mix_assign_w_meta %>%
  left_join(., POP_meta_ss) %>%
  group_by(HaulID, StationID, StartLatitudeDD, StartLongitudeDD, DonatingLab, CollectionYear, CollectionMonth, CollectionDay, DepthM) %>%
  tally() %>%
  arrange(n) %>%
  filter(n > 10) %>%
  ...
  filter(CollectionYear == "1999") %>%
  mutate(StartLongitudeDD = ifelse(StartLongitudeDD > 0, StartLongitudeDD*-1, StartLongitudeDD))
# damn, looks like the Longitude sign is flipped in one entry and that's causing the data duplication.

# Still need to perfect the metadata for this project once Juliana has sorted through all the issues.
```


```{r fix-metadata-issues-in-the-assignment-output}
assignments_meta_fixed <- mix_assign_w_meta %>%
  mutate(DepthM = ifelse(DepthM == 0, NA, DepthM)) %>%
  mutate(StartLongitudeDD = ifelse(StartLongitudeDD > 0, StartLongitudeDD*-1, StartLongitudeDD)) %>%
  mutate(StartLongitudeDD = ifelse(StartLongitudeDD == 0, NA, StartLongitudeDD)) %>%
...
hauls <- assignments_meta_fixed %>%
  ...
  left_join(., assignments_meta_fixed) %>%
...
ggsave("pdf_outputs/POP_genetic_group_count_by_haul.pdf", width = 6, height = 4)
...
# ensure that each haul is only entered once.
tmp_pie <- hauls %>%
  ungroup() %>%
...
tmp_pie$collection <- factor(tmp_pie$collection, levels = c("A", "B", "C", "D"))

for_plotting <- tmp_pie %>%
...
Relationship between haul size and longitude?
```{r haul-size-vs-longitude}
hauls %>%
  group_by(haul, collection, StartLongitudeDD) %>%
  tally(name = "n_form") %>%
  group_by(haul) %>%
  mutate(haul_size = sum(n_form)) %>%
  ungroup() %>% 
  ggplot(aes(x = StartLongitudeDD, y = haul_size)) +
  geom_point() +
  theme_bw() 
...
goa_map_zoom +
  geom_scatterpie(aes(x = StartLongitudeDD,y = StartLatitudeDD, r = 0.5),
                   data = for_plotting, cols = colnames(for_plotting[, c(6:9)]), alpha = 0.6) +
scale_fill_manual(values = c( "darkcyan","deepskyblue", "darkblue", "salmon")) +
...
Making the scatterpies round!
```{r fix-pies-propotions}
library(ggthemes)
...
testp1 <- countries %>% 
  ...
  geom_map(map = world_map, fill = "gray") +
  coord_equal() +
  theme_bw() +
  scale_x_continuous(limits = c(-182, -125), breaks = c(-180, -160, -140), labels = c("180°W", "160°W", "140°W")) +
  scale_y_continuous(limits = c(47, 65), breaks = c(50, 60), labels = c("50°N", "60°N")) +
  labs(x = "",
       y = "")

testp1 +
  geom_scatterpie(aes(x = StartLongitudeDD,y = StartLatitudeDD, r = 0.8),
                   data = for_plotting, cols = colnames(for_plotting[, c(6:9)]), alpha = 0.75) +
  scale_fill_manual(values = c("darkcyan","deepskyblue", "darkblue", "salmon")) +
  coord_equal() +
  labs(fill = "genetic \ngroup")

ggsave("pdf_outputs/POP_bottomtrawl_haul_pies.pdf", width = 8, height = 3)
```
Although that works, the overplotting is pretty terrible.
I can pull the PDF into Inkscape to fix the overplotting - apparently there's no easy mechanism to do so in R with jittering.

```{r}
for_plotting %>%
  filter(StartLongitudeDD < -160) %>%
  filter(StartLongitudeDD > -170) %>%
  filter(D < 0.5)

```
hauls 8 and 13 have the exact sample lat/lon and collection year and only slightly different proportions. 
Go up to the metadata to see if there is anything different about the Station numbers - especially since the depth data is missing.
...
ggplot() +
  geom_scatterpie(aes(x = StartLongitudeDD,y = DepthM/-10, r = 1),
                   data = plot_pie_data, cols = colnames(for_plotting[, c(6:9)]), alpha = 0.6) +
  scale_fill_manual(values = c("darkcyan", "deepskyblue", "darkblue", "salmon" )) +
  ...
  # annotate("text", x = -178, y = -37, label = "deeper", size = 5) +
  # annotate("segment", x = -178, xend = -178, y = -32, yend = -36, color = "black", arrow = arrow(length = unit(0.2, "cm"))) +
  # annotate("text", x = -172.5, y = -38, label = "west", size = 5) +
  # annotate("segment", x = -165, xend = -170.5, y = -38, yend = -38, color = "black", arrow = arrow(length = unit(0.2, "cm"))) +
  scale_x_continuous(breaks = c(-180, -160, -140), labels = c("180°W", "160°W", "140°W")) +
  scale_y_continuous(breaks = c(-40, -30, -20, -10), labels = c("400", "300", "200", "100"))


ggsave("pdf_outputs/trawl_proportions_longitude_depth.png", width = 6, height = 3.5)
```
...
### Looking at new metadata that Juliana organized

```{r cleaned-up-metadata}
# compare the new metadata with the previous version.
new_meta <- read_xlsx("../data/POP_database_export_May2023_updated_Jan2024.xlsx", sheet = "Sheet1")

# so many parsing errors. Ugh
# what if I try reading it in as a .csv?
