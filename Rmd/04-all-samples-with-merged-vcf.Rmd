...
hauls %>%
  ggplot(aes(x = haul, fill = collection)) +
  geom_bar(stat = "count") +
  theme_bw() +
  scale_color_manual(values = c("darkcyan", "deepskyblue", "salmon", "darkblue", "sienna", "slateblue", "darkorange","firebrick", "purple4")) +
  theme(
...
for_plotting <- hauls %>%
  ungroup() %>%
  select(haul, indiv, StartLatitudeDD, StartLongitudeDD, collection) %>%
  group_by(haul) %>%
...
goa_map +
  geom_scatterpie(aes(x = StartLongitudeDD,y = StartLatitudeDD, r = 1),
                   data = for_plotting, cols = colnames(for_plotting[, c(4:7)]), alpha = 0.6) +
...
ggsave("pdf_outputs/haul_proportions.pdf", width = 6, height = 6)
```
...
goa_map_zoom +
  geom_scatterpie(aes(x = StartLongitudeDD,y = StartLatitudeDD, r = 0.5),
                   data = for_plotting, cols = colnames(for_plotting[, c(4:7)]), alpha = 0.5) +
  scale_fill_manual(values = c("darkcyan", "deepskyblue", "salmon", "darkblue", "sienna", "slateblue", "darkorange","firebrick", "purple4")) +
theme(
  axis.text = element_text(size = 8),
  axis.title.x = element_text(margin = margin(t = 10)),
